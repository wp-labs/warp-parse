...
fn load_sinks(work_root: &str) -> RunResult<Sinks> {
    let eng_conf = Arc::new(EngineConfig::load_or_init(work_root).err_conv()?);
    Ok(Sinks::new(work_root, eng_conf))
}

pub fn list_sinks(args: SinksCommonArgs) -> RunResult<()> {
    let sinks = load_sinks(&args.work_root)?;
...
pub fn show_sink_routes(args: SinksRouteArgs) -> RunResult<()> {
    let sinks = load_sinks(&args.common.work_root)?;
