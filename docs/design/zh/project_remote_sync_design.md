...
- 状态: Draft
- 范围: `wproj conf update`、`wparse restart`、HTTP admin API 运行时指令
...
`wproj conf update` 与 `wparse restart` 必须复用同一个核心模块。
...
- 同步目标是明确的发布版本，而不是移动中的分支头
- 本次动作的目标版本必须解析到唯一发布 tag
- 本地工作区不干净时直接失败

这样可以保证“更新规则”对应的是一个稳定、可审计的发布版本，而不是某个随时间移动的 branch head。
...
- `wproj conf update` 可以完成首次同步和后续更新
