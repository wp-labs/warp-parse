...
use super::{
    ...
    LOCK_PATH, MODEL_VERSION_PATH, RULE_MAPPING_PATH, STATE_PATH,
};
...
pub fn capture_project_remote_snapshot<P: AsRef<Path>>(
    ...
    let model_version_file = read_optional_file(&work_root.join(MODEL_VERSION_PATH))?;
...
pub fn restore_project_remote_update<P: AsRef<Path>>(
    ...
    restore_optional_file(
        &work_root.join(MODEL_VERSION_PATH),
        snapshot.model_version_file.as_deref(),
    )?;
    Ok(())
}
